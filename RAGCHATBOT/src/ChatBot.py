...
from injest import load_PDF_docs
...
## This will also search the data after loading the PDF documents
...
## ChatBOT query loop to keep asking for queries until user types 'exit'

    while True:
        query = input("Enter your query to search (exit to quit):\n")
        if query == "exit": break
        summary = rag_search_pdf.search_and_summarize(query, top_k=10)
        print("Final ChatBOT Summary:", summary)
