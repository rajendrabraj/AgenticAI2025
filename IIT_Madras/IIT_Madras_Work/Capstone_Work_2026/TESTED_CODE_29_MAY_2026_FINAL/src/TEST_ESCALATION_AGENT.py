...
filename = os.path.join(logs_directory_path, "agent_esclation_LOG.log") 
...
def main() -> None:
    while True :
        ...
        logging.info("Escalation Agent : Search KnowledgeBase Response : %s", json.dumps(clean_output, indent=4))            
        # Convert JSON object to JSON string
        response_string = json.dumps(response_json, indent=4)            
        print(response_string)              
        ## Execute and run the PII Agent now
        print("Calling the Escalation Agent")
