...
def main() -> None:
    while True :
        ...
        logging.info("PII Query Search KnowledgeBase Response : %s", json.dumps(clean_output, indent=4))
        ...
        print("Calling the PII Agent")
        logging.info(f"Calling PII  Agent Now for query {query}")
        query= user_input    
        final_PII_Check_response = run_PII_check(query,response_string)
        print(final_PII_Check_response)
        logging.info(f"PII Check  Result : {final_PII_Check_response}")
        
        # ## Execute Cyber Attack Check 
        # print("Calling Cyber ATTACK Check Agent")
        # logging.info(f"Calling PII  Agent Now for query {query}")
        # query= user_input    
        # final_cyber_check = execute_cyber_checks(query,response_string)
        # print(final_PII_Check_response)
