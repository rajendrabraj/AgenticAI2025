...
if PINECONE_AVAILABLE:
    # Vector stores will be initialized lazily when an endpoint actually uses Pinecone.
    vector_store = None


def get_embeddings():
    global embeddings, document_embeddings
    ...
    if document_embeddings is None:
        try:
            ...
            logging.info("Document embeddings created successfully")
        except Exception as exc:
            print(f"Embedding initialization failed: {exc}")
            logging.info(f"Embedding initialization failed: {exc}")
            document_embeddings = []
    return embeddings
...
def init_pinecone_vector_store():
    ...
    if not pinecone_api_key:
        logging.info("PINECONE_API_KEY not found, skipping Pinecone vector store initialization.")
        return None
    try:
        ...
        logging.error(f"Pinecone initialization failed: {exc}")
        return None
...
def retrieve_docs(query):
    ...
    if not document_embeddings:
        logging.warning("No document embeddings available; returning fallback documents.")
        return documents[:3]
...
## Define the Agents and Tasks lazily
...
## Define helper functions to create crews as needed

def get_support_crew():
    agents = create_core_agents()
    tasks = create_core_tasks(agents)
    return Crew(
        agents=[
            agents["intent_agent"],
            agents["safety_agent"],
...
def run_experiment(prompt_task: Task, query: str):
    agents = create_core_agents()
    tasks = create_core_tasks(agents)
    runtime_prompt_task = Task(
        description=prompt_task.description,
        agent=agents["resolution_agent"],
        expected_output=prompt_task.expected_output,
    )
    crew = Crew(
        agents=[
            agents["intent_agent"],
...
def run_without_retrieval(query):
    ...
    response = get_llm().invoke(simple_prompt)
...
def run_with_retrieval(query):
    return get_support_crew().kickoff(inputs={"query": query})
...
def run_experiment_memory(query: str, memory: str):
    crew = get_evaluation_crew()
    return crew.kickoff(inputs={"query": query, "memory": memory})
...
def run_experiment_memory_with_context(query: str, memory: str, feedback_context : str ):
    crew = get_evaluation_crew()
    return crew.kickoff(inputs={"query": query, "memory": memory, "feedback_context": feedback_context})
...
def execute_deployment_tracking(query):
    ...
    support_crew_Evaluation = get_evaluation_crew()
...
def main() -> None:
    ...
    query_first= "I was charged twice for my subscription"    
    logging.info("START- Running #10 Query Test for FAST API started with query: " + query_first)
    execute_deployment_tracking(query_first)
    logging.info("END- Running #10 Query Test for FAST API started with query: " + query_first)
    logging.info("END- Running #10 FAST API loaded successfully")
