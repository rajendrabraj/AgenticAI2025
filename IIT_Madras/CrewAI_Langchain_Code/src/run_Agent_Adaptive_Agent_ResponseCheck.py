...
def evaluate_final_verdict(agent_result):
    """
    Checks the final output of the agent workflow
    and returns PASS or FAIL status.
    """

    result_text = str(agent_result).lower()

    # Keywords indicating successful execution
    success_keywords = [
        "success",
        "passed",
        "compliant",
    ...
    # Keywords indicating failure/issues
    failure_keywords = [
        "fail",
        "failed",
        "unsafe",
    ...
    success_score = sum(word in result_text for word in success_keywords)
    failure_score = sum(word in result_text for word in failure_keywords)

    final_verdict = "PASS" if success_score >= failure_score else "FAIL"

    return {
        "Final Verdict": final_verdict,
        "Success Score": success_score,
        "Failure Score": failure_score,
        "Agent Output": str(agent_result)
    }
...
#     print(result)
#     print("-" * 90)
#     logging.info("BASIC Crew Agent executed successfully")
#     logging.info(f"BASIC Crew Agent Result: {result}")
...
## Executing the : Multi Turn Conversation functions and Memory

    logging.info("START : Run #7 Evaluation Agent with Memory Conversation History")

    print("-" * 90)
    print("START: Run #7 Evaluation Agent with Memory Conversation History")
    
    query_one = user_input
    
    logging.info("With Query : Run #7 " + query_one )      
    
    response_memory_1 = run_memory_conversation(query_one)
    print(query_one)
    print("-" * 90)
    print(response_memory_1)
    print("-" * 90)
    logging.info(f"Result: {response_memory_1}")
    print("-" * 90)

    query_two = user_input
    logging.info("With Query : Run #7 " + query_two )  
    
    print("-" * 90)
    response_memory_2 = run_memory_conversation(query_two)
    print("-" * 90)
    print(query_two)
    print(response_memory_2)
    print("-" * 90)
    logging.info(f"Result: {response_memory_2}")
    print("-" * 90)

    print("-" * 90)

    query_three = user_input
    logging.info("With Query : Run #7 " + query_three )  
    logging.info("========================")   
    print("-" * 90)
    response_memory_3 = run_memory_conversation(query_three)
    print("-" * 90)
    print(query_three)
    print(response_memory_3)
    print("-" * 90)
    logging.info(f"Result: {response_memory_3}")
    print("-" * 90)
    print("END: Run #7 Evaluation Agent with Memory Conversation History")
    print("-" * 90)
    logging.info("END : Run #7 Evaluation Agent with Memory Conversation History")

    ##Running the Crew agents to TEST before and AFter Context.
    
    print("START: Run #8 : Test Evaluation Agent with FEEBACK and Context :  ")
    print("-" * 90)
    logging.info("START: Run #8 : Test Evaluation Agent with FEEBACK and Context : ")

    query= user_input
    
    response_before_with_context = run_conversation_memory_context(query)
    print("-" * 90)
    print(f"BEFORE FEEDBACK Result with Context  : {response_before_with_context}")
    logging.info(f"BEFORE FEEDBACK  Result with Context  : {response_before_with_context}")
    logging.info("========================")   
    print("-" * 90)
    ...
    store_feedback(
        query= user_input,
        response=response_before_with_context,
        rating=2,
        comments="Response was unclear and did not explain refund process"
    )

    ##SIMULATE AFTER FEEDBACK
    query= user_input
    response_after_with_context = run_conversation_memory_context(query)

    print("-" * 90)
    logging.info(f"AFTER/BEFORE FEEDBACK Query Used   : {query} ")
    logging.info("========================")   
    print(f"AFTER FEEDBACK Result with Context: {response_after_with_context}")    
    logging.info(f"AFTER FEEDBACK  Result with Context  : {response_after_with_context}")
    logging.info("========================")   
    print("-" * 90)

    print("END: Run #8 : Test Evaluation Agent with FEEBACK and Context :  ")
    print("-" * 90)
      
    # response = run_experiment_memory_with_context(
    #         query="I want refund for my duplicate payment",
    #         memory="Customer had billing issue before",
    #         feedback_context="Previous response was incomplete"
    #     )
    print("-" * 90)
    evaluation_results = evaluate_final_verdict(response_after_with_context)
    print(evaluation_results)
    logging.info(f"Run #8 : Test Evaluation Agent Query   : {query} ")
    logging.info("========================")   
    logging.info(f"Test Evaluation Agent : Run #8 : Results for Evaluation : {evaluation_results} ")  
    logging.info("========================")     
    print("-" * 90)
